document.addEventListener("DOMContentLoaded", () => {
  ...
  function update3DButtonState() {
    ...
    const hasBack =
      postcardData.message && postcardData.message.trim().length > 0;
  ...
  if (btnFront && btnBack) {
    ...
    btnBack.addEventListener("click", () => {
      ...
      if (charCount) {
        const currentLen = postcardData.message
          ? postcardData.message.length
          : 0;
        charCount.style.color =
          currentLen < 5 || currentLen >= 130 ? "#e74c3c" : "var(--text-sub)";
      }
    ...
    modeButtons.forEach((btn) => {
      btn.addEventListener("click", (e) => {
        modeButtons.forEach((b) =>
          b.classList.remove("constructor-mode-active"),
        );
  ...
  const updateDisplay = (is3DMode = false, botSenderData = null) => {
    ...
    if (postcardData.currentSide === "front") {
      ...
      previewContent.style.cursor = postcardData.frontImage
        ? "grab"
        : "default";

      if (postcardData.frontImage) {
        const hintHTML = is3DMode
          ? ""
      ...
      const avatarStyle = avatar
        ? `background-image: url(${avatar}); background-size: cover; background-position: ${posX}% ${posY}%;`
        : "";
      const avatarContent = avatar ? "" : initial;

      const date = new Date().toLocaleDateString("en-GB");
      const cardID =
        sender.userId || "PJ-" + Math.floor(1000 + Math.random() * 9000);
    ...
    btnGenerateAI.onclick = async () => {
      ...
      if (state.energy < 100) {
        return showToastNotification(
          "⚡ Not enough energy (need 100)! Visit the Shop or wait for tomorrow.",
        );
      }
      ...
      try {
        ...
        // ==========================================================
        // ВНЕДРЕНА ЗАГЛУШКА ДЛЯ ЛИЦЕВОЙ СТОРОНЫ
        // ==========================================================
        clearInterval(progressInterval);
        console.warn("AI Generation failed. Using fallback image...", e);

        progressBar.style.width = "100%";
        progressPercent.innerText = "100%";

        const fallbackImages = [
          "https://images.unsplash.com/photo-1488085061387-422e29b40080?w=1200&h=800&fit=crop", // Дорога
          "https://images.unsplash.com/photo-1469854523086-cc02fe5d8800?w=1200&h=800&fit=crop", // Карта
          "https://images.unsplash.com/photo-1476514525535-07fb3b4ae5f1?w=1200&h=800&fit=crop", // Горы
          "https://images.unsplash.com/photo-1507525428034-b723cf961d3e?w=1200&h=800&fit=crop", // Пляж
        ];

        state.energy -= 100; // Списываем энергию, так как картинку все равно даем
        updateEnergyUI();

        postcardData.frontImage =
        ...
        if (typeof showToastNotification === "function") {
          showToastNotification(
            "⚠️ AI Server is busy. Applied a beautiful fallback image!",
          );
        }
      } finally {
  ...
  if (stampGrid && aiStampConstructor && btnGenerateStamp) {
    ...
    btnGenerateStamp.addEventListener("click", async () => {
      ...
      if (!userPrompt)
        return showAppAlert("Please describe what you want on your stamp.");
      if (state.energy < 50)
        return showToastNotification(
          "⚡ Not enough energy (need 50)! Visit the Shop or wait for tomorrow.",
        );
      ...
      try {
        ...
        // Классная винтажная картинка-марка в качестве заглушки
        postcardData.stampImage =
          "https://images.unsplash.com/photo-1596401057633-54a8fe8ef647?w=400&h=500&fit=crop";
        postcardData.stampType = "ai";
        updateDisplay();
        promptInput.value = "";
        ...
        if (typeof showToastNotification === "function") {
          showToastNotification(
            "⚠️ AI Server is busy. Applied a fallback vintage stamp!",
          );
        }
      } finally {
  ...
  if (btnView3D) {
    btnView3D.onclick = function () {
      ...
      updateDisplay(true);
      frontDiv.innerHTML = "";
      frontDiv.appendChild(
        previewContent.querySelector("#postcard-canvas").cloneNode(true),
      );
      ...
      const w =
        rect.width > 0 ? rect.width : Math.min(window.innerWidth * 0.9, 500);
      const scale3D = w / 600;
  ...
  function imgDragStart(e) {
    if (postcardData.currentSide !== "front" || !postcardData.frontImage)
      return;
    isDraggingImg = true;
  ...
  if (previewContent) {
    ...
    previewContent.addEventListener("touchstart", imgDragStart, {
      passive: false,
    });
  ...
  function dragEnd() {
    ...
    if (inner) {
      inner.style.transition =
        "transform 0.6s cubic-bezier(0.25, 0.8, 0.25, 1), box-shadow 0.5s ease";
  ...
  document.addEventListener("click", (e) => {
    ...
    const cardData = isSent
      ? state.sentPostcards[index]
      : state.receivedPostcards[index];
    if (!cardData) return;
    ...
    if (!isSent && cardData.isNew) {
      ...
      if (
        flagsTitle &&
        document.getElementById("modal-flags").style.display === "flex"
      ) {
        const continent = flagsTitle.textContent
          .replace("📥", "")
          .replace("📤", "")
          .trim();
        if (typeof openFlagsModal === "function")
          openFlagsModal(continent, "received");
      }
    }
    ...
    postcardData.stampType =
      cardData.stampType || (cardData.stampImage ? "ai" : "emoji");
    ...
    frontDiv.innerHTML = "";
    let fCanvasClone = document
      .getElementById("postcard-canvas")
      .cloneNode(true);
    ...
    const miniAvatarEl = document
      .getElementById("postcard-canvas")
      .querySelector(".sender-mini-avatar");
    if (miniAvatarEl) {
      if (!isSent) {
  ...
  // ==========================================================================
  // ВРЕМЕННАЯ ЛОГИКА ДЛЯ БЫСТРОЙ ОТПРАВКИ (ТЕСТИРОВАНИЕ)
  // Вставляется строго ВНУТРИ DOMContentLoaded!
  // ==========================================================================
  const quickSendBtn = document.getElementById("test-quick-send-btn");
  if (quickSendBtn) {
    quickSendBtn.addEventListener("click", () => {
      // 1. Имитируем вытягивание адреса из облака (если ты забыл его вытянуть)
      if (!state.currentTarget && typeof countryList !== "undefined") {
        const randomCountry =
          countryList[Math.floor(Math.random() * countryList.length)];
        state.currentTarget = {
          targetId: "Bot_Tester_" + Math.floor(Math.random() * 9999),
          name: "Auto Tester",
      ...
      // 2. Наполняем ТВОЮ локальную переменную postcardData идеальными данными
      // Используем красивую заглушку с Unsplash
      postcardData.frontImage =
        "https://images.unsplash.com/photo-1476514525535-07fb3b4ae5f1?w=600&h=400&fit=crop";
      postcardData.message =
        "Hello from the Quick Send button! This is an automatic test message. 🚀🌍";
      postcardData.font = "'Caveat', cursive";
      postcardData.color = "#1e3799";
      postcardData.stamp = "🚀";
      postcardData.stampType = "emoji";
      postcardData.stampImage = null;

      // 3. Заполняем физические поля, чтобы UI тоже обновился
      if (document.getElementById("card-message")) {
        document.getElementById("card-message").value = postcardData.message;
      }

      // Перерисовываем холст, чтобы игра увидела новые данные
      updateDisplay();

      // 4. Эмулируем клик по НАСТОЯЩЕЙ зеленой кнопке отправки
      // Она проверит postcardData, увидит, что всё заполнено, и запустит всю цепочку!
      const realSendBtn = document.getElementById("send-card-btn");
      if (realSendBtn) {
        realSendBtn.click();
      }
    });
