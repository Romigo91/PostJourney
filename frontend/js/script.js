  ...
  try {
    ...
    if (typeof showCustomAlert === "function") {
      showCustomAlert(
        "⚠️",
        "Storage Full!",
        "Cannot save more images. Your browser memory is full.<br><br>Use <b>'Clear All Data'</b> in Settings.",
        "#e74c3c",
      );
    } else {
  ...
  if (tagsContainer) {
    ...
    if (myTags.length > 0) {
      myTags.forEach((tag) => {
        ...
        tagDiv.style.background = "var(--bg-input)";
        tagDiv.style.border = "1px solid var(--border)";
        tagDiv.style.borderRadius = "12px";
        tagDiv.style.color = "var(--text-sub)";
...
function refreshAllLists() {
  ...
  if (emptyText) {
    emptyText.style.display =
      state.tracking && state.tracking.length > 0 ? "none" : "block";
  }
  ...
  renderListComponent("tracking-list", state.tracking, (item) => {
    ...
    if (mapBadge) {
      const incomingCount = state.tracking.filter(
  ...
  const validSentPostcards = state.sentPostcards.filter((card) => {
    ...
    if (
      dest.includes("personal") ||
      dest.includes("collection") ||
      dest.includes("offline") ||
      dest === ""
    ) {
      return false;
    }
  ...
  const uniqueCountriesCount = new Set(
    validSentPostcards.map((card) => card.countryFlag || card.flag),
  ).size;
  ...
  const botEntries = state.bots.map((bot) => {
    const realCountriesCount = bot.contactedCountries
  ...
  const cardTemplate = (card, index, isReceived) => {
    ...
    const bgImg = card.frontImage
      ? `background-image: url(${card.frontImage});`
      : `background: #eee;`;
  ...
  const renderArchiveList = (containerId, dataArray, isReceived = false) => {
    ...
    container.innerHTML = [...dataArray]
      ...
      .map((item, i) => {
        ...
        const bgImg = item.frontImage
          ? `background-image: url(${item.frontImage});`
          : `background: #eee;`;
  ...
  window.openGalleryModal = function (isReceived, filterFlag = null) {
    ...
    const fullArray = isReceived
      ? state.receivedPostcards
      : state.sentPostcards;
    const mappedArray = fullArray.map((card, index) => ({
      card,
      originalIndex: index,
    }));
    ...
    const filteredArray = filterFlag
      ? mappedArray.filter(
          (item) => (item.card.countryFlag || item.card.flag) === filterFlag,
        )
      : mappedArray;
    ...
    grid.innerHTML = filteredArray
      .map((item) => {
        const flag = item.card.countryFlag || item.card.flag || "🌍";
        const bgImg = item.card.frontImage
          ? `background-image: url(${item.card.frontImage});`
          : `background: #eee;`;
...
<div class="archive-card ${pulseClass}" data-index="${item.originalIndex}" data-is-sent="${!isReceived}" 
  ...
  if (sentCountEl)
    sentCountEl.textContent = state.sentPostcards
      ? state.sentPostcards.length
      : 0;

  const receivedCountEl = document.getElementById("received-count");
  if (receivedCountEl)
    receivedCountEl.textContent = state.receivedPostcards
...
window.openFlagsModal = function (continent, tab) {
  ...
  const validSentPostcards = state.sentPostcards.filter(
    (c) =>
      c.isOffline !== true && c.to && !c.to.toLowerCase().includes("personal"),
  );
  const sentFlags = [
    ...new Set(validSentPostcards.map((c) => c.countryFlag || c.flag)),
  ];
  ...
  grid.innerHTML = continentFlags
    .map((f) => {
      ...
      let flagClass = isCollected
        ? "flag-circle flag-collected"
        : "flag-circle flag-locked";
      ...
      const clickAction = isCollected
        ? `onclick="openGalleryModal(${isReceived}, '${f}')"`
        : "";
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  navItems.forEach((btn) => {
    btn.onclick = () => {
      ...
      screens.forEach((scr) =>
        scr.classList.toggle("screen-active", scr.dataset.screen === target),
      );
  ...
  if (avatarEl) {
    ...
    function startDrag(e) {
      const editBtn = document.getElementById("edit-profile-btn");
      const isEditingNow =
        editBtn && editBtn.getAttribute("data-mode") === "save";
...
document.addEventListener(
  "DOMContentLoaded",
  window.updateTrackingPreviewWidget,
);
...
function renderEditTagsModal() {
  ...
  const selectBox = document.getElementById("interest-select-modal");
  if (!container) return;
  ...
  if (selectBox) {
    if (tempModalTags.length >= 3) {
      ...
      selectBox.options[0].text = "Max 3 interests reached";
    } else {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (avatarInputModal) {
    avatarInputModal.addEventListener("change", function () {
  ...
  if (saveBtn) {
    saveBtn.addEventListener("click", () => {
      state.profile.name = document
        .getElementById("input-name-modal")
        .value.trim();
      state.profile.bio = document
        .getElementById("input-bio-modal")
        .value.trim();
      ...
      if (countrySelect) {
        ...
        if (countryObj) {
          state.profile.country = countryObj.name;
          state.profile.countryFlag = countryObj.flag;
        }
      }
